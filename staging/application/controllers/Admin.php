...
// 		$data['created_date'] = Date("Y-m-d H:i:s");
	...
	function get_longitude_latitude_from_adress($address){
        ...
        $long = 0;
         ...
         $address = str_replace(" ", "+", $address);
          try {
         $json = file_get_contents('https://maps.google.com/maps/api/geocode/json?address='.$address.'&key=AIzaSyAC9RuigAHsx7QDLc-t41UhtGBCkoyrxDE');
     ...
     public function addplumber_longlat()
	{ 
	
    	$data['address_latlong'] = $this->common_m->get_address_latlong();
    	
    	 foreach ($data['address_latlong'] as $row) {
    	      echo $email = $row['email'];   echo "****rrrrrrrr";
    	     echo $id = $row['user_id'];
    	     echo "****";
    	    echo $complete_address= $row['address'].' '.$row['city'].' '.$row['zip_code'].' US';
    	  
    	    echo "---------##";
    	   
    	   $array  = $this->get_longitude_latitude_from_adress($complete_address);
    	   echo  $latitude  = $array['lat'];
    	   echo "---------##";
    	   
    	   echo  $longitude = $array['lng'];   
    ...
    public function InsertProducts($products,$data1)
{
    ...
    $insert[$headings['response_time']]                       = $product['response_time'];   
       $insert[$headings['first_name']]                = $product['first_name'];
         $insert[$headings['last_name']]                = $product['last_name'];
      ...
      $insert[$headings['name_on_card']]                = $product['name_on_card'];
      $insert[$headings['country_code']]                       ="+1";      
          $insert[$headings['country']]                 =   $product['country']; 
