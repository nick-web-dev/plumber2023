...
//   echo "<pre>";print_r($emails);exit;
    ...
    public function listed_plumber()
    {
        ...
        if ($this->session->userdata('auth_level')) {
            if ($this->session->userdata('auth_level') == 9) {
                ...
                if ($_POST['page_count'] != ""){
                    $rowcount = $_POST['page_count'];
                    $config["per_page"] = $rowcount;
                }
                else{
                    $config["per_page"] = 20;
                }
                $data['page_count_get'] = $_POST['page_count'];
    ...
    public function plumber_import_csv($id = "")
    {
        if ($this->session->userdata('auth_level')) {
            ...
            if ($this->session->userdata('auth_level') == 9) {
                ...
                $this->load->view('admin/includes/header', $data);
                $this->load->view('admin/includes/footer');
                $this->load->view('admin/import_plumber_csv', $data);
            } else {
                redirect(base_url() . 'home/index');
            }
        } else {
    ...
    public function upload_plumber_events()
    {
        $error = '';
        $total_line = '';
        // session_start();
    ...
    public function import_plumber()
    {
    ...
    public function process_plumber()
    {
